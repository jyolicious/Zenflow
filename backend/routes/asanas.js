...
// optional admin middleware (see below)
const { requireAdmin } = require('../middleware/auth') // implement this
...
router.get('/', async (req, res) => {
  try {
    ...
    const limit = Math.min(Math.max(parseInt(req.query.limit || '50', 10), 1), 100) // 1..100
...
router.put('/:id', requireAdmin, async (req, res) => {
  try {
    ...
    if (req.body.tags) updates.tags = Array.isArray(req.body.tags) ? req.body.tags.map(t => String(t).trim()) : []
    ...
    const updated = await Asana.findByIdAndUpdate(id, updates, { new: true, runValidators: true }).lean()
